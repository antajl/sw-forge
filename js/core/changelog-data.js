...
const STATIC_CHANGELOG = [
  ...
  },
  {
    ...
    items: {
      ...
      fr: [
        ...
        'Rechargement : graphiques du tableau de bord en taille finale immédiatement (sans animation) ; la table des runes se remplit peu après via planification idle.',
        'Démarrage : ne charge plus la démo intégrée avant le slot actif (corrige le double rendu au F5).',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    items: {
      ...
      fr: [
        'Portraits de monstres, icônes de compétences, leader skills, sets de runes, éléments et types d\'artefacts chargés depuis le bundle du site quand disponible — visites répétées plus rapides, moins de dépendance aux CDN externes.',
        'Monstres → Plan de compétences : libellés Stockage corrigés, portraits sur compétences bloquées, colonnes « skill-ups nécessaires » ; Roster Filtres avancés → Pas au max (skill-ups). Tableaux Artefacts & Reliques : icônes de type (SWARFARM). Onglet Forge Score : curseur de distribution fonctionnel.',
        'Comptes partagés : bannière Keep/Sell, résumé roster et jusqu\'à trois noms d\'unités sans runes ou avec skill-ups ; onglet Équipes liste leurs escouades partagées.',
        'Ouvrir le SWEX d\'un autre en lecture seule via lien de partage, URL ?profile= JSON ou export ?data= dans la barre d\'adresse.',
        'Artefacts → Table : colonne Emplacement affiche les noms de monstres (ex. Lushen), pas les numéros #ID.',
        'Étoiles des monstres éveillés à nouveau roses avec un contour légèrement plus net.',
        'Chargement plus rapide après export : démo en cache, une feuille de style bundlée, vérification des runes possible en arrière-plan.',
        'Interface française chargée uniquement si vous choisissez FR ; polices hébergées sur le site (sans Google Fonts).',
      ],
    },
  },
  {
    ...
    sections: [
      {
        ...
        items: [
          ...
          'Teams: battle SPD + totem from SWEX; optional speed buff toggle, clearer slowest-slot gap',
        ],
      },
      {
      ...
      },
      {
        ...
        phase: 'Medium',
        lead: 'Read-only share links already work. Next: handle very large JSON exports.',
        items: [
          'Full "all" share for large accounts (Worker limit workaround)',
        ],
      },
      {
        id: 'runes',
      ...
      },
      {
        ...
        title: 'Gear advice',
        phase: 'Medium',
        lead: 'Gear tables and inventory work; Keep/Sell rules for artifacts and relics are implemented.',
        items: [
    ...
    sections: [
      {
        ...
        items: [
          ...
          'Teams: боевой SPD + тотем из SWEX; опционально — баф скорости в бою, нагляднее разрыв с самым медленным слотом',
        ],
      },
      {
        id: 'monsters-dashboard',
      ...
      },
      {
        ...
        title: 'Чужой аккаунт',
        phase: 'Средний',
        lead: 'Ссылки «только просмотр» уже есть. Дальше — поддержка очень больших JSON.',
        items: [
          'Полный share «all» для больших аккаунтов (обход лимита Worker)',
        ],
      },
      {
        id: 'runes',
        ...
        items: [
          ...
          'Опционально жёстче правило Grind (настройка скорости и сабы Hit Rate)',
        ],
      },
      {
        id: 'monsters-roster',
        kicker: 'Монстры',
        title: 'Глубина ростера',
        phase: 'Низкий',
        items: [
          'Теги контента на юнитах (RTA, Siege, ToA) и фильтр по тегам',
        ],
      },
      {
        id: 'artifacts',
        ...
        phase: 'Средний',
        lead: 'Таблицы и инвентарь работают; правила Keep/Sell для артефактов и реликтов реализованы.',
        items: [
          'Опционально отдельная верхняя вкладка, или остаться под Runes',
          'Больше подтверждённых имён типов реликтов от комьюнити',
          'Иконки реликтов: скриншоты из игры → PNG в assets/relics/ (вручную)',
        ],
      },
      {
        id: 'builders',
